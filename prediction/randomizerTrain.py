...
# is accurate enough
human_control = [2,3,4,5,7,7,9,10,12,13,14,15,17,18,19,20,22,23,24,26,27,28,30,31,31,33,34,35,37,38,39,41,42,43,45,46,47,49,50,51,52,54,55,57,58,59,60,62,63,64,66,67,68,69,70,70,70,71,72,72,74,75,77,78,79,80,82,83,84,86,87,88,90,91,92,94,95,95,96,97,98,99,100,101,103,104,106,107,107,108,109,111,112]
simple_control = [1,2,3,4,5,6,7,8,8,9,10,11,12,13,14,14,15,16,17,18,19,20,21,21,22,23,24,25,26,27,28,28,29,30,31,32,33,34,35,35,36,37,38,39,40,41,42,42,43,44,45,46,47,48,49,49,50,51,52,53,54,55,56,56,57,58,59,60,61,62,63,63,64,65,66,67,68,69,70,70,71,72,73,74,75,76,77,77,78,79]

human_avg   = predict(human_pred)
simple_avg  = predict(simple_pred)
simple_control_avg = predict(simple_control)
human_control_avg  = predict(human_control)
...
print('Average: ', avg)
print('\nSimple control: ', simple_control_avg)
print('Human control: ', human_control_avg)
...
# if (Control Average - Average) > Average then it's human, otherwise its not human
isControlHuman = human_control_avg-avg > avg
isControlSimple = simple_control_avg-avg > avg

isAccurate = isControlHuman == True and isControlSimple != True
# Should return true
print('🧠 Human Control is human: ', isControlHuman)
print('🤖 Simple Control is human: ', isControlSimple)
print(f'\n{"✔️" if isAccurate else "❌"}  Algorithm is {"accurate" if isAccurate else "not accurate"}')
